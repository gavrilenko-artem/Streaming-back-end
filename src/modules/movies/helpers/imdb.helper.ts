...
const findCrewMember = (crew: CrewMember[], memberJob: string) =>
  crew.find(({ job }) => job === memberJob).name || ''
...
export const IMDBRequests = () => {
  ...
  return {
    getMovie: (IMDBId: string) =>
      axios.get<IMDBMovie>(`${MOVIE_URL}/${IMDBId}?${queryParams}`),
    getMovieCredits: (IMDBId: number) =>
      axios.get<GetCreditsResponse>(
        `${MOVIE_URL}/${IMDBId}/credits?${queryParams}`
      ),
    searchMovie: (query: string) =>
      axios.get<SearchMoviesResponse>(
        `${MOVIE_URL}/search/movie?${queryParams}&query=${query}`
      ),
    getTrailer: (IMDBId: number) =>
      axios.get<GetTrailerResponse>(
        `${MOVIE_URL}/${IMDBId}/videos?${queryParams}`
      ),
  }
}
const { getMovieCredits, getTrailer } = IMDBRequests()
